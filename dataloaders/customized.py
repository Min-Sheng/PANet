...
def getCellMask(label):
    ...
    """
    # Dense Mask
    fg_mask = torch.where(label == 128,
                          torch.ones_like(label), torch.zeros_like(label))
    contour_mask = torch.where(label == 255,
                          torch.ones_like(label), torch.zeros_like(label))
...
def cell_fewShot(paired_sample, n_ways, n_shots, cnt_query):
    """
    Postprocess paired sample for fewshot settings
    ...
    ###### Generate query mask for each semantic class (including FG, Contour, and BG) ######
    query_masks = [[getCellMask(query_label) for query_label in query_labels]]
...
def fsscell_fewshot(base_dir, class_id_table_path, transforms, to_tensor, labels, n_ways, n_shots, max_iters,
                n_queries=1):
    """
    Args:
        base_dir:
        ...
        n_queries:
            number of query images
    """
    fss_cell = FSSCell(base_dir=base_dir, class_id_table_path=class_id_table_path, \
                        transforms=transforms, to_tensor=to_tensor)
    fss_cell.add_attrib('basic', attrib_basic, {})

    # Load image ids for each class
    sub_ids = []
    for label in labels:
        sub_id = []
        for id_ in fss_cell.ids:
            if fss_cell.class_id_table[f'{label}'] in id_:
                sub_id.append(id_)
        #print(len(sub_id))
        sub_ids.append(sub_id)
    # Create sub-datasets and add class_id attribute
    subsets = fss_cell.subsets(sub_ids, [{'basic': {'class_id': cls_id}} for cls_id in labels])
    ...
    paired_data = PairedDataset(subsets, n_elements=n_elements, max_iters=max_iters, same=False,
                                pair_based_transforms=[
                                    (cell_fewShot, {'n_ways': n_ways, 'n_shots': n_shots,
                                               'cnt_query': cnt_query})])
    return paired_data

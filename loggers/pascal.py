...
class VOCLogger(BaseLogger):
    """The logger for the segmentation task of PASCAL dataset.
    """
    ...
    def _add_images(self, iter_, sample_batched, query_pred):
        ...
        support_fg_mask = make_grid(sample_batched['support_mask'][0][0]['fg_mask'].float(), nrow=1, normalize=True, scale_each=True, pad_value=1)
        support_mask_cat = torch.cat((sample_batched['support_mask'][0][0]['bg_mask'], sample_batched['support_mask'][0][0]['fg_mask']), dim=0)

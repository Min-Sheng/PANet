    ...
    def forward(self, supp_imgs, fore_mask, cntr_mask, back_mask, qry_imgs, cnt_query):
        ...
        for epi in range(batch_size):
            ...
            if self.config['align'] and self.training:
                align_loss_epi, supp_preds = self.alignLoss(qry_fts[:, epi], pred, supp_fts[:, :, epi],
                                                fore_mask[:, :, epi], cntr_mask[:, :, epi], back_mask[:, :, epi], cnt_query)
                align_loss += align_loss_epi
                support_outputs.append(supp_preds)

        output = torch.stack(outputs, dim=1)  # N x B x (1 + 2 x Wa) x H x W
        output = output.view(-1, *output.shape[2:])
        if self.config['align'] and self.training:
            support_output = torch.stack(support_outputs, dim=2) # Wa x Sh x B x H x W
            return output, align_loss / batch_size, support_output
        else:
    ...
    def alignLoss(self, qry_fts, pred, supp_fts, fore_mask, cntr_mask, back_mask, cnt_query):
        ...
        Args:
            ...
            pred: predicted segmentation score
                expect shape: N x (1 + 2 * Wa) x H x W
        ...
        binary_masks = [pred_mask == i for i in range(1 + n_classes)] # (1 + 2 * Wa) x [N x 1 x H' x W']
        skip_ways = [i for i in range(n_ways) if binary_masks[i * 2 + 1].sum() == 0]
        pred_mask = torch.stack(binary_masks, dim=1).float()  # N x (1 + 2 * Wa) x 1 x H' x W'
        # (N x 1 x C x H' x W') * (N x (1 + 2 * Wa) x 1 x H' x W') = (N x (1 + 2 * Wa) x C x H' x W')
        qry_prototypes = torch.sum(qry_fts.unsqueeze(1) * pred_mask, dim=(0, 3, 4)) # (1 + 2 *Wa) x C
        qry_prototypes = qry_prototypes / (pred_mask.sum((0, 3, 4)) + 1e-5)  # (1 + 2 *Wa) x C
        ...
        for way in range(n_ways):
            ...
            if way in skip_ways:
                for shot in range(n_shots):
                    empty = torch.zeros(([3, ] + list(fore_mask.shape[-2:])),
                                        device=fore_mask.device).float()
                    supp_preds_per_way.append(empty)
            else:
                # Get the query prototypes
                prototypes = [qry_prototypes[[0]], qry_prototypes[[way + 1]], qry_prototypes[[way + 2]]]
                for shot in range(n_shots):
                    ...
                    supp_pred = F.interpolate(supp_pred, size=fore_mask.shape[-2:],
                                            mode='bilinear') # 1 x 3 x H x W
                    # Construct the support Ground-Truth segmentation
                    supp_label = torch.zeros_like(fore_mask[way, shot],
                                                device=img_fts.device).long() # H x W
                    ...
                    supp_preds_per_way.append(supp_pred.squeeze())
            supp_preds_per_way = torch.stack(supp_preds_per_way, dim = 0) # Sh x 3 x H x W
            supp_preds.append(supp_preds_per_way)
        supp_preds = torch.stack(supp_preds, dim = 0) # Wa x Sh x 3 x H x W
        return loss, supp_preds
